...
export function BusRoutes(){
  ...
  function assembleRoutes(){
    ...
    // If it is looping, the end stop sequence is 1;
    if(isLoopService===false){
    for(let num=start1; num<=end1; num++){
      ...
      // loop through the length of tempList1
      const len1 = tempList1.length;
      for(let num=1; num<=len1; num++){
        tempItem = tempList1.find((i)=>{
          return (i["StopSequence"] === num)
      })
        // loop service can skip sequence number
        console.log("In loop , item ", tempItem);
        if(!tempItem){
          continue;
        }
      ...
      // manually push the last item in the loop
      route1.push(dest1); // for loop, origin===dest.
    }
